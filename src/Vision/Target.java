...
public class Target {
	...
	public double getDimensionRatio() { // should be close to target dimensions
		double dimensionRatio = Math.abs((15 / 2 - lRect.height / rRect.width))
				+ Math.abs((15 / 2 - rRect.height / lRect.width));
		// dimension ratio cross-match
		return dimensionRatio;
	}

	/*
	 * ratios: target is 15.3 inches tall target is 2 inches wide Target
	 * ft/Target pixels = FOV ft/ FOV pixels FOV Yft = 2 * w = 2 * d * tan theta
	...
	public double getXDistance() { // right relative to robot is positive
		...
		double pixelOffset = ((lRect.x + rRect.x + rRect.width) / 2 - xRes / 2); // positive
																					// is
																					// right
