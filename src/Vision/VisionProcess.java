...
public class VisionProcess {
				...
				if (boundingRects.get(i).height > 37 && boundingRects.get(j).height > 37) { // filter
					...
					if (boundingRects.get(i).width > 5 && boundingRects.get(j).width > 5) {
						if (boundingRects.get(i).x > boundingRects.get(j).x) { // i
							...
							targets.add(new Target(boundingRects.get(i), boundingRects.get(j), xRes, yRes,
									horizontalVAngle, verticalVAngle));
						}
					}
				}
	...
	private void rateTargets() {
		...
		for (int i = 0; i < targets.size(); i++) { // here's the real
													// "algorithm"
	...
	public Mat getOutput(Mat input) {
		...
		this.output = input;
		Scalar color1 = new Scalar(0, 255, 0); // color picking depends on
												// source image
		...
		Imgproc.rectangle(this.output, // overall target
				...
				new Point(currentTarget.getRRect().x + currentTarget.getRRect().width,
						currentTarget.getRRect().y + currentTarget.getRRect().height),
				color1, 2);
		Imgproc.rectangle(this.output, new Point(currentTarget.getLRect().x, currentTarget.getLRect().y),
				new Point(currentTarget.getLRect().x + currentTarget.getLRect().width,
						currentTarget.getLRect().y + currentTarget.getLRect().height),
				color2, 2);
		...
		// crosshairs
		Imgproc.line(this.output, new Point(getXRes() / 2 - 40, yRes / 2), new Point(getXRes() / 2 + 40, yRes / 2),
				color1, 2);
		Imgproc.line(this.output, new Point(getXRes() / 2, yRes / 2 - 40), new Point(getXRes() / 2, yRes / 2 + 40),
				color1, 2);
		return output;
	}
